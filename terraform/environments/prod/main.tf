locals {
  ...
  # Compartilhado entre module.api (deploy principal, atrás do IAP) e o
  # deploy interno em scheduler.tf (mesma imagem/código, sem IAP -- só os 2
  # endpoints internos são chamados ali) -- extraído pra local pra não
  # duplicar/divergir entre os dois.
  api_env_vars = {
